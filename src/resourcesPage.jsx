    ...
    <div className={`min-h-screen flex flex-col ${darkMode ? 'bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 text-gray-100' : 'bg-gradient-to-br from-pink-100 via-purple-100 to-indigo-100 text-gray-900'} font-sans transition-all duration-300`}>
      ...
      <main className="flex-grow container mx-auto mt-8 px-4">
        <div className={`rounded-lg shadow-xl p-6 ${darkMode ? 'bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-md' : 'bg-white bg-opacity-70 backdrop-filter backdrop-blur'}`}>
          <h1 className={`text-5xl font-extrabold mb-7 text-center ${darkMode ? 'text-transparent bg-clip-text bg-gradient-to-r from-pink-300 via-purple-300 to-indigo-300' : 'text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500'}`}>
            SDE Resources
          </h1>
...
function ResourceTab({ icon, label, tab, activeTab, setActiveTab, darkMode }) {
  return (
    <Button
      ...
      className={`${activeTab === tab ? (darkMode ? 'bg-pink-500 text-white hover:bg-pink-600' : 'bg-purple-600 text-white hover:bg-purple-700') : 'bg-transparent'}`}
...
function ResourceLink({ href, title, description, darkMode, category }) {
  return (
    ...
    >
      <h3 className={`text-lg font-semibold mb-2 ${darkMode ? 'text-pink-400' : 'text-purple-600'}`}>{title}</h3>
